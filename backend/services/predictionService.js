...
function buildUserMessage(payload) {
  ...
  return `Analyze this person's goal and predict their likelihood of failure:
...
export function validatePredictionPayload(body) {
  const {
    goal,
    hours,
    energy,
...
export async function runOpenAiPrediction(payload) {
...
export function runDemoPrediction(payload) {
  const {
    goal,
    hours,
    energy,
  ...
  const risk_level =
    failure_probability >= 65 ? "high" : failure_probability >= 40 ? "medium" : "low";
